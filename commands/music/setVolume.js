    ...
    async run(interaction) {
        ...
        try {
            if (queue.node.volume === volume) {
                await interaction.reply(`The volume is already set to \`${volume}\`.`);
            } else {
                queue.node.setVolume(volume);
                await interaction.reply(`The volume is now set to \`${volume}\`.`);
            }
        } catch (error) {
            console.error(error);
            await interaction.reply("An error occurred while playing the previous music.");
        }
    }
};
